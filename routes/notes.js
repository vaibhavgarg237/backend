...
//Route 4: Update note: PUT "/api/notes/updatenote". Login required
...
//Route 5: Delete note: Delete "/api/notes/deletenote". Login required
router.delete("/deletenote/:id", fetchusers, async (req, res) => {
  try {
    //fetch note from db for deletion
    //what if note doesn't exists
    let note = await Notes.findById(req.params.id);
    if (!note) {
      return res.status(404).send("Not found");
